...
Varyings Vertex(Attributes input)
{
	...
	//GRADIENT_EXTENSION
	#if defined(GRADIENT_COLOR_EXTENSION_ON)
		#if defined(USE_WORLD_COORDINATES_FOR_GRADIENT)
			// output.pos = UnityObjectToClipPos(input.vertex);
		#else
			// output.worldPos = input.texcoord0;
		#endif
		#if defined(LIGHTMAP_ON)
			output.worldPos = input.texcoord1;
		#endif
		#if defined(DYNAMICLIGHTMAP_ON)
			output.worldPos = input.texcoord2;
		#endif
	#endif
...
half4 Fragment (Varyings input, half vFace : VFACE) : SV_Target
{
	...
	#if defined(GRADIENT_COLOR_EXTENSION_ON)
		float adjustedWorldY = input.texcoords.y;
		#if defined(USE_WORLD_COORDINATES_FOR_GRADIENT)
			#if defined(USE_REALTIME_WORLD_POSITION_FOR_GRADIENT)
				adjustedWorldY = input.worldPos.y;
				adjustedWorldY -= _parentY;
			#else
				adjustedWorldY = input.texcoords.y;
			#endif
		#endif
		gradient = lerp(_colorEnd, _colorMid, adjustedWorldY / _middle) * step(adjustedWorldY, _middle);
		gradient += lerp(_colorMid, _colorStart, (adjustedWorldY - _middle) / (1 - _middle)) * step(_middle, adjustedWorldY);
	#endif
	...
	// Output color
	half3 color = albedo.rgb * lightColor.rgb * ramp * gradient;
