...
{
    public static int UnitPercent()
    {
        return Random.Range(0, 1);
    }
    public static float UnitPercentFloat()
    {
    ...
    public static float PercentFloat()
    {
...
public static class ConversionExtensions
{
    ...
    public static Vector3 GetWorldPositionByScreenPosition(Vector3 screenPos, Camera camera, Transform cameraTransform, Transform cameraParent)
    {
        Ray mousePos = camera.ScreenPointToRay(screenPos);
        Plane ground = new Plane(cameraTransform.transform.forward, cameraParent.position.z * Vector3.forward);
...
{
    public static bool RandomPoint(Vector3 center, float range, out Vector3 result, float minRange = 0f)
    {
        for (float r = 0; r < 1f; r += 0.1f)
        {
            for (int i = 0; i < 10; i++)
            {
                Vector3 randomPoint = center + Random.insideUnitSphere * Mathf.Clamp((r * range), minRange, range * 2f);
                NavMeshHit hit;
    ...
    public static bool RandomPointRanged(Vector3 center, float range, out Vector3 result)
    {
        for (int i = 0; i < 30; i++)
        {
